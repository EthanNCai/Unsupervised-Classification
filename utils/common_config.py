...
def get_model(p, pretrain_path=None):
    """
        input -> config dictionary,
                 path to the pretrain model file(optional)

        output -> model
    """

    """
    Step 1: get the model according to the configuration file.
    """
    ...
    if p['backbone'] == 'resnet18':
        ...
        if 'imagenet' in p['train_db_name']:
            ...
            # backbone acquired here is a dict like {'backbone': ResNet, 'dim' : Int}

    else:
        raise ValueError('Invalid backbone {}'.format(p['backbone']))

    """
    Step 2: setting up the Contrastive if on stage 1 or Cluster model if on stage 2 
    using the backbone model we just acquired
    """
    ...
    """ 
    Step 3(Optional): load pretrained weights
    """
    if pretrain_path is not None and os.path.exists(pretrain_path):
        state = torch.load(pretrain_path, map_location='cpu')

        if p['setup'] == 'scan':
            # Weights are supposed to be transferred from contrastive training
            missing = model.load_state_dict(state, strict=False)
            """ 
            missing here is a tuple
            missing[0] contains the successful loaded params
            missing[1] contains the unsuccessful params.
            """
            ...
            # Weights are supposed to be transferred from scan
            # We only continue with the best head
            # (pop all heads first, then copy back the best head)
...
def get_val_transformations(p):
    # why is that the transformation of val and train data is not kept identical?.
    return transforms.Compose([
